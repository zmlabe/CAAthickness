...
def readPiomas(directory,years,threshold):
    ...
    threshold : float
        mask snow cover amounts < to this value
    ...
    ### Reshape into [year,month,lat,lon]
        ...
        if months != 12:
            ...
            print 'SNC data available through ---> "%s"' % month
            print 'SNC data available from ---> (%s - %s)' \
            ...
            ValueError('Issue with reshaping SNC array from binary')
    
    ### Mask out threshold values
    var[np.where(var < threshold)] = np.nan
    print 'Masking SNC data < %s m!' % threshold
