...
Usage
-----
    sitnew,lats,lons = regridPiomas(directory,sic,lat1,lon1,lat2,lon2)
"""

print('\n>>> Using regridPiomas function!')

import numpy as np
import read_SeaIceConcentration_PIOMAS as CS
...
### Call functions
lat1,lon1,sic = CS.readPiomas(directorydata1,years,0.0)
...
def regrid(lat1,lon1,lat2,lon2,var,years):
    ...
    for i in range(varn.shape[0]):
        for j in range(varn.shape[1]):
...
def netcdfPiomas(lats,lons,var,directory):
    ...
    name = 'SeaIceConcentration/piomas_regrid1x1_sic_LENS_19792017.nc'
    filename = directory + name
    ncfile = Dataset(filename,'w',format='NETCDF4')
    ncfile.description = 'PIOMAS sic from 1979-2017 ' \
    ...
    ### Units
    varns.units = 'fraction of 1'
    ncfile.title = 'PIOMAS sic on LENS Grid 1x1 degree'
    ...
    ### Data
    years[:] = list(range(var.shape[0]))
    months[:] = list(range(var.shape[1]))
    latitude[:] = lats
    longitude[:] = lons
    varns[:] = var
...
m = Basemap(projection='npstere',boundinglat=55,lon_0=270,
...
cs = m.contourf(lon2,lat2,var[:,:]*100.,
                values,latlon=True)
cs1 = m.contour(lon2,lat2,var[:,:]*100.,
...
plt.savefig('/home/zlabe/Desktop/' + 'testing_regrid_PIOMAS_sicnew.png',dpi=300)
